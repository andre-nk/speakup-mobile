...
class MaterialCustomizerRepository {
  ...
  Future<Material> customizeAndStoreMaterial(String userID, String nativeLanguage, String goal, String level) async {
    try {
...
'''}
      ...
      final materialData = jsonDecode(response.output!.substring(7, response.output!.length - 4));
      final customizedMaterial = Material.fromJson(materialData);

      //store to Firestore
      FirebaseFirestore.instance
          .collection('users')
          .doc(userID)
          .collection('customized_materials')
          .doc(customizedMaterial.id)
          .set(customizedMaterial.toJson());
